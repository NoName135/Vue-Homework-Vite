...
export default {
  ...
  watch: {
    article() {
      ...
      const timeStamp = new Date(this.tempArticle.create_at * 1000)
      this.createAt =`${timeStamp.getFullYear()}-${(timeStamp.getMonth()+1).toString().padStart(2,0)}-${timeStamp.getDate().toString().padStart(2,0)}`;
    },
    createAt() {
